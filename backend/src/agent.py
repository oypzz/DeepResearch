...
class DeepResearchAgent:
    ...
    def __init__(self, config: Configuration | None = None) -> None:
        ...
        # 压缩效果评估器
        self._evaluator = CompressionEvaluator()
        self._evaluator.set_llm(self.llm)  # 设置LLM用于召回率/精确率评估
        self.summarizer.set_evaluator(self._evaluator)
    ...
    def run(self, topic: str) -> SummaryStateOutput:
        ...
        # 保存压缩效果评估报告
        try:
            eval_path = self._evaluator.save_report()
            logger.info("Compression evaluation report saved to: %s", eval_path)
        except Exception as exc:  # pragma: no cover - 防御性容错
            logger.warning("Failed to save evaluation report: %s", exc)
    ...
    def run_stream(self, topic: str) -> Iterator[dict[str, Any]]:
        ...
        try:
            ...
            for thread in threads:
                ...
                if thread.is_alive():
                    logger.warning("Thread %s did not finish within 60s timeout", thread.name)

            # 清理RAG资源
            try:
                self.summarizer.cleanup()
            except Exception as exc:  # pragma: no cover - 防御性容错
                logger.warning("Failed to cleanup summarizer: %s", exc)

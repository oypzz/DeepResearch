...
class ToolCallTracker:
    ...
    def _extract_note_id(self, response: str) -> Optional[str]:
        ...
        # 方式2: JSON格式 '"id": "xxx"'
        match = re.search(r'"id":\s*"([^"]+)"', response)
        if match:
            return match.group(1).strip()
        ...
        match = re.search(r"note_([^.]+)\.md", response)
        if match:
            return match.group(1)

        logger.warning("Could not extract note_id from response: %s", response[:200])
        return None
